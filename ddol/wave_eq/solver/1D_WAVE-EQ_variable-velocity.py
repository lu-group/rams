...
def I(x):
    ...
    """
    return np.exp(-(x-0.5)**2/0.01) * np.sin(np.pi*x)
...
plt.contourf(grid_t,grid_x,U.transpose(), cmap = 'jet', levels = 100)
plt.colorbar()
# anim1 = viz_tools.anim_1D(X,U, dt, 10, save = True , myxlim = (0, 1.5) , myylim = (-0.5,1.5))
plt.show()