...
# plt.show()
# Plot the contour of the last snapshot
...
y = np.array(y)
tU = np.array(tU)
# plt.contour(x,y,tU, cmap="coolwarm")
# plt.colorbar()
# plt.show()
# plt.figure()
plt.imshow(tU, cmap="jet", origin='lower', extent=[x.min(), x.max(), y.min(), y.max()])  # 确保数据与坐标轴正确对应
plt.colorbar()  # 显示颜色条
plt.show()
